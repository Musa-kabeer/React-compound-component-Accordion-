import AccordionProvider from './ui/AccordionsContext';
// import Accordions from './ui/AccordionsContext';
const items = [
  {
    id: 1,
    ...
    <div className="container">
      <ul>
        <AccordionProvider>
          {items.map((item) => (
            <li key={item.id}>
              <AccordionProvider.Header acccordionId={item.id}>
                {item.header}
              </AccordionProvider.Header>

              <AccordionProvider.Body acccordionId={item.id}>
                {item.body}
              </AccordionProvider.Body>
            </li>
          ))}
        </AccordionProvider>
      </ul>
